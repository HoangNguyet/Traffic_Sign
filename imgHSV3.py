...
def binaryImg(img):
    ...
    cv.imshow("Red BinarCy Image", b_img_red)
    cv.waitKey(0)
...
def is_specific_shape(contour):
    ...
    approx = cv.approxPolyDP(contour, 0.02 * perimeter, True) # Hoặc 0.025, 0.03
    num_vertices = len(approx)

    # print(f"Debug Shape: Vertices={num_vertices}") # Debug

    if num_vertices == 8:
        # Kiểm tra thêm độ tròn cho bát giác
        area = cv.contourArea(contour)
        circularity = 4 * np.pi * (area / (perimeter ** 2)) if perimeter > 0 else 0
        # print(f"Debug Octagon: Circularity={circularity:.2f}")
        if circularity > 0.75: # Ngưỡng độ tròn cho bát giác (cần tinh chỉnh)
            return "octagon"
    # Bạn có thể thêm logic cho hình thoi ở đây nếu muốn phân biệt rõ ràng với hình chữ nhật
    # Ví dụ: một hình thoi lồi 4 đỉnh, nhưng không phải tất cả các góc đều là 90 độ.
    # Hoặc kiểm tra tỷ lệ đường chéo. Điều này phức tạp hơn.
    # if num_vertices == 4 and cv.isContourConvex(approx):
    # # Thêm logic kiểm tra hình thoi
    # return "diamond"
    return None
...
#         #Tính hình chữ nhật bao quanh: Trả về x: Tọa độ x góc trên bên trái hình chữ nhật; y: Tọa độ y góc trên bên trái; w: Chiều rộng; h: chiều cao hình chữ nhật
...
def findSigns(frame):
    ...
    for c in contours_red + contours_blue:
        ...
        print(f"Area: {area}, Shape: {shape}, Circle: {is_circle}, Specific: {specific_shape}")
